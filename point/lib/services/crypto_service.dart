...
class CryptoService {
  ...
  /// or null if MLS is not yet ready for this group (key exchange pending).
  /// Callers must skip the send when null is returned — never send plaintext.
  Future<String?> encrypt(String serverGroupId, Map<String, dynamic> data) async {
    if (!isInitialized) {
      debugPrint('[Crypto] Not initialized — skipping relay for $serverGroupId');
      return null;
    }
    final crypto = _requireCrypto();
    final gid = _groupIdMap[serverGroupId];
    if (gid == null) {
      debugPrint('[Crypto] No MLS group for $serverGroupId — skipping relay (key exchange pending)');
      return null;
    }
